...
1. **ASI(エージェント)との接続関係が明確**: LLM01=ASI01/T6(目標乗っ取り)の土台、LLM06 Excessive Agency=ASIのLeast-Agency思想そのもの、LLM08=ASI06(RAG汚染・漏出)、LLM05 Improper Output=ASI05(RCE)。\*\*「LLM層で理解→エージェント層で増幅を評価」\*\*という二層の読み筋が、ASI/T&Mと完全に整合。
1. **起点①の裏取りが厚くなった**: LLM02(機密漏えい)+LLM07(システムプロンプト漏えい)+LLM08(ベクトル漏出)。特に**LLM08のクロスコンテキスト漏出は[B-1のクロステナント・ベクトル漏出防止](../docs/to-be/03-three-pillars-to-be.md)と同一論点**。permission-aware vector storeが中立標準でも必須要件と確認できた=確度が高い。
1. **起点②の具体化**: LLM05(出力監視)・LLM10(リソース消費監視/Denial of Wallet)。**LLM10のコスト枯渇は監視ダッシュボード(S4)のKPIに「コスト異常」を入れる根拠**。
1. **起点③の裏取り**: LLM09(誤情報・ハルシネーション・**実在しないコードライブラリ提案**)+LLM05(不適切な出力処理)+LLM02(出力経由の著作権/IP漏えい)。文化庁の生成物適法性確認と、LLM09の「unsafe code generation」が接続(ライセンススキャンF6の補強)。
1. **「LLM出力をゼロトラストで扱う」が明文(LLM05)**: [00のゼロトラスト原則](../docs/to-be/00-principles-and-scope.md)を、入力(自然言語)だけでなく**出力にも**拡張する根拠。出力検証=基本指針3の技術的裏付け。
1. **システムプロンプトに秘密を置かない(LLM07)**: 自社でGeminiエージェントを構築する際の設計規範。認証情報・内部ルール・権限構造をプロンプトに書かない=開発者向けの禁止事項候補(S3で具体化)。
1. **CC BY-SA 4.0で翻案可**: ASI/T&Mと同ライセンス。外販テンプレで「LLM単体リスク早見表」として和訳・自社化できる(継承表記要)。
