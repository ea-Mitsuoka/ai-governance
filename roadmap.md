...
```
[S0 土台] → [S1 外部基準の理解] → [S2 理想形の探索] → [S3 自社ルールの策定] → [S4 技術実装と運用自動化]
   ↑                                                                                    │
   └──────────────────── 改訂ガバナンス・ループ(継続的に S1→S4 を回す)────────────────┘
```
...
> **現状(2026-06-13)**: ナレッジ運用ルール ✅運用中 / 用語集 ✅`docs/glossary.md` 作成済 / RACI表 ⏸️体制未確定で保留 / As-Is棚卸し ⏸️体制確定・既存規程の提供待ち。**S0は3/4が完了、残り2件は体制依存で着手不可**。
...
- [ ] NIST AI RMF 1.0(Govern / Map / Measure / Manage)+ Generative AI Profile
- [ ] ISO/IEC 42001(AIマネジメントシステム)/ ISO/IEC 23894(AIリスク管理)
- [ ] EU AI Act(リスク分類・禁止用途・GPAI義務)— 外販で海外/グローバル企業を想定する場合
- [ ] OECD AI原則

**技術・脅威カタログ(S4の伏線)**

- [ ] Google SAIF(Secure AI Framework)— Gemini Enterprise前提で最重要
- [x] OWASP Top 10 for LLM Applications / OWASP Agentic AI threats(※Agentic 2本=ASI Top10 + T&M v1.1を読込済。LLM Top10単体の要約は未)
...
**成果物**: `sources/*.md`(各基準の要約)、`docs/framework-comparison.md`(横断比較表)〔追加:複数基準を1枚で見渡す対照表〕。

> **現状(2026-06-13)**: 国内必須(AI事業者GL・広島・文化庁)+エージェント脅威カタログ ✅読込済。**完了条件の必須3件(NIST AI RMF / ISO 42001・23894 / Google SAIF)が未読**(ユーザー添付待ち)。横断比較表 `docs/framework-comparison.md` ❌未作成=**既読分だけで先行作成が可能**。
...
> **現状(2026-06-13)**: `docs/to-be/` に00〜04+索引が揃い、起点3点+横断7テーマすべてTo-Be記述済 ✅。**S2はほぼ完了**。`docs/risk-taxonomy_ideal.md` は独立ファイル未作成だが、内容は[01のリスク台帳分類体系](docs/to-be/01-risk-classification-and-grading.md)に記述済=独立ファイル化するかは任意。
