...
## CNAPP 技術範囲の凡例(各分類がCNAPPのどこに位置するか)

本表は CNAPP 製品の比較にとどまらず、Gemini 統制の全体像を扱うため、**意図的に CNAPP の範囲外(SIEM・FinOps・ガバナンス等)を含む**。各分類見出しに以下のタグを付す。

- 🟢 **CNAPP中核** … CSPM / CIEM / CWPP / DSPM / KSPM / IaC。CNAPPそのもの。
- 🔵 **CNAPP拡張** … AI-SPM・サプライチェーン(Wiz Code)等、近年CNAPPが取り込みつつある領域。
- ⚪ **CNAPP範囲外** … SIEM/SOC・FinOps・ガバナンス・法務・SI等の別領域。**製品が×になりやすく、そのまま e-Agency のサービス提供余地**になる。

> **補足:** CNAPPの本質は「可視化・検出(ポスチャ管理)」。**Model Armor(インライン遮断)/ SIEM(集約)/ SSPM(SaaS保護)はいずれもCNAPPではない別カテゴリ**である点に注意(添付のCNAPP定義図でもSSPMはCNAPPの外)。
...
### ⚪ 【組織ガバナンス層】(コンサル・ルール策定 → **製品では埋まらない=e-Agency サービスの主戦場**) — **CNAPP範囲外**(GRC・コンサル領域)
...
### 🟢 【ID・アクセス管理 / ネットワーク境界(CIEM / CSPM)】 — **CNAPP中核**(CSPM/CIEM)

> **CNAPP範囲外の行:** SSO・Workforce Identity Federation=IAM/Identity領域、VPC-SC・Private Service Connect設計=ネットワークアーキ設計(CSPMは"監視"はするが"設計"はしない)。
...
### 🟢 【データ保護層(DSPM)】 — **CNAPP中核**(DSPM)

> **CNAPP範囲外の行:** CMEK/EKM・HSM鍵管理設計=KMS(鍵管理)領域、DPIA・生成AIの法務/著作権(起点③)=法務・プライバシー領域。
...
### 🔵 【AI入出力・運用層(AI-SPM / CWPP)】 — **CWPP=CNAPP中核 / AI-SPM=CNAPP拡張 / その他は範囲外**

> **CNAPP範囲外の行:** Model Armor(インラインAIガードレール=LLMファイアウォール)、AIレッドチーミング(診断サービス)、Confidential AI(インフラ機能)はいずれもCNAPPではない別技術。
...
### ⚪ 【監査・運用・SIEM層】 — **CNAPP範囲外**(SIEM/SOC・GRC・監査。CNAPPは"ここへデータを送る側")
...
### ⚪ 【FinOps(AI投資)層】 — **CNAPP範囲外**(コスト管理。セキュリティとは別分野)
...
### ⚪ 【導入基盤】 — **CNAPP範囲外**(SI/構築。IaC"スキャン"は中核だがLZ"構築"は別)
...
### 🟢🔵⚪ 【新規・大分類不適合項目】(CI/CD・開発・相関分析) — **混在**

> **内訳:** Security Graph=CNAPP中核の相関エンジン🟢 / Wiz Code(サプライチェーン)=CNAPP拡張🔵 / Mandiant自動プレイブック・TI=脅威インテリジェンス・SOC領域で**CNAPP範囲外**⚪。
